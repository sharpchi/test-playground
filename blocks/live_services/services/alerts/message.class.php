 ...
 */
    ...
    public function runCronGroupDeliver() {
        ...
        $blockRecord = get_record_sql($sql);
        $lastCron = $blockRecord->lastcron;
        $this->groupDeliverForModules(array('quiz','assignment',''), $lastCron);
        $timenow = time();
        $sql = "UPDATE {$CFG->prefix}block SET lastcron = $timenow WHERE name = 'live_services'";
        execute_sql($sql);
    }
    ...
    private function groupDeliverForModules($modules, $lastCron) {
        ...
        foreach($modules as $module) {
            $sql = "SELECT e.name, e.timestart, e.timeduration, e.courseid, e.description FROM {$CFG->prefix}event e WHERE timemodified > $lastCron AND e.modulename='$module' AND e.courseid > 1";
