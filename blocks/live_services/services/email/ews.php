...
class EWSWrapper
{
	...
	public function GetUnreadEMail( $liveId, $exchangeServiceAuthData, $count )
	{
		...
		try
		{
            ...
            $authToken              = $exchangeServiceAuthData['AuthenticationData']['SAMLToken'];

            if( strlen($authToken) > 0 )
            {
                $returnValue = new EWSQueryResult( $this->GetUnreadEMailCount($liveId, $serviceEndPoint, $authToken) );

                if( $returnValue->getCount() > 0)
                {
                    // The the SOAP Envelope for the UnreadEmail count
                    $soapEnvelope = $this->GetUnreadEmailEnvelope( $liveId, $serviceEndPoint, $authToken, $count );
    ...
    public function GetEmailItem( $liveId, $exchangeServiceAuthData, $itemId, $changeKey )
    {
		...
		try
		{
            ...
            if( strlen($authToken) > 0 )
            {
                ...
                $soapEnvelope = $this->GetViewEmailEnvelope( $liveId, $serviceEndPoint, $authToken, $itemId,$changeKey);
                // echo $soapEnvelope;
                    ...
                    if( $responseCodeNode[0] == "NoError" )
                    {
                        ...
                        if((bool)$hasAttachments[0])
                        {
                            ...
                            foreach($fileAttachments as $fileAttachment)
                            {
                                ...
                                $attachmentsContent = $this->GetAttachment($liveId, $serviceEndPoint, $authToken, (string)$attachmentId[0]);
    ...
    public function GetCalendarItem( $liveId, $exchangeServiceAuthData, $itemId, $changeKey )
    {
		...
		try
		{
            ...
            {
                // Get the SOAP Envelope that will be used to Send Email
                $soapEnvelope = $this->GetViewCalendarEnvelope( $liveId, $serviceEndPoint, $authToken, $itemId,$changeKey);
                // echo $soapEnvelope;
	...
	public function GetUpcomingCalendarItems( $liveId, $exchangeServiceAuthData, $count )
	{
		...
		try
		{
            ...
            {
                // The the SOAP Envelope for the UnreadEmail count
                $soapEnvelope = $this->GetUpcomingCalendarItemsEnvelope( $liveId, $serviceEndPoint, $authToken, $count );
    ...
    public function SendMailWithAttachments($liveId, $exchangeServiceAuthData, $to, $subject, $body, $attachments)
    {
		...
		try
		{
            ...
            {
                // Get the SOAP Envelope that will be used to Send Email
                $soapEnvelope = $this->GetCreateEmailEnvelope( $liveId, $serviceEndPoint, $authToken, $to, $subject,$body);
                // echo $soapEnvelope;
                ...
                if( count($responseCodeNode) > 0 )
                {
                    if( $responseCodeNode[0] == "NoError" )
                    {
                        ...
                        $changeKey = $changeKeyNode[0];
                            try
                            {
                                $attachmentArray = $this->CreateAttachments($liveId, $serviceEndPoint, $authToken, $attachments, $id, $changeKey);
                                ...
                                $resultString = $this->SendSavedMailWithAttachments($liveId, $serviceEndPoint, $authToken, $id, $changeKey);
                                return $resultString;
                            }
                            catch(Exception $e)
                            {
     ...
     * @param <string> $authToken - A SAML Token
     * @param <string> $attachmentId - the id that uniquely identfies the attachment
     * @return <array>
     */

    private function GetAttachment($liveId, $serviceEndPoint, $authToken, $attachmentId)
    {
        try
        {
            $soapEnvelope = $this->GetAttachmentEnvelope($liveId, $serviceEndPoint, $authToken, $attachmentId);
            // POST The SOAP Envelope to the URL for the Exchange Server
            $curl = new CurlLib();
            $response = $curl->postSoapEnvelope( $serviceEndPoint,  $soapEnvelope, array('Content-Type: text/xml; charset=utf-8'));
            $xml = simplexml_load_string($response);
    ...
    public function SendMailForward($liveId, $exchangeServiceAuthData, $id, $changeKey, $to, $subject, $body)
    {
		...
		try
		{
            ...
            {
                // Get the SOAP Envelope that will be used to Send Email
                $soapEnvelope = $this->GetSendEmailForwardEnvelope( $liveId, $serviceEndPoint, $authToken, $id, $changeKey, $to, $subject, $body);
    ...
    public function SendMailReply($liveId, $exchangeServiceAuthData, $id, $changeKey, $to, $subject, $body)
    {
		...
		try
		{
            ...
            {
                // Get the SOAP Envelope that will be used to Send Email
                $soapEnvelope = $this->GetSendEmailReplyEnvelope( $liveId, $serviceEndPoint, $authToken, $id, $changeKey, $to, $subject, $body);
     ...
     * @return <array>
     */
    private function CreateAttachments($liveId, $serviceEndPoint, $authToken, $attachments, $id, $changeKey)
    {
        $code = ''; $reason = ''; $error = ''; $exceptionmessage = '';
        // POST The SOAP Envelope to the URL for the Exchange Server
        $curl = new CurlLib();
        foreach($attachments as $attachment)
        {
            try
            {
                $soapEnvelope = $this->GetCreateAttachmentEnvelope($liveId, $serviceEndPoint, $authToken, $id, $changeKey, $attachment['name'], $attachment['content']);
                $response = $curl->postSoapEnvelope( $serviceEndPoint,  $soapEnvelope, array('Content-Type: text/xml; charset=utf-8'));
                $xml = simplexml_load_string($response);
    ...
    private function SendSavedMailWithAttachments($liveId, $serviceEndPoint, $authToken, $id, $changeKey)
    {
            $code = ''; $reason = ''; $error = ''; $exceptionmessage = '';
        // POST The SOAP Envelope to the URL for the Exchange Server
            $curl = new CurlLib();
            try
            {
                $soapEnvelope = $this->GetSendEmailWithAttachmentsEnvelope($liveId, $serviceEndPoint, $authToken, $id, $changeKey);
                $response = $curl->postSoapEnvelope( $serviceEndPoint,  $soapEnvelope, array('Content-Type: text/xml; charset=utf-8'));
                $xml = simplexml_load_string($response);
                $xml->registerXPathNamespace( "s", "http://schemas.xmlsoap.org/soap/envelope/" );
                $xml->registerXPathNamespace( "m", "http://schemas.microsoft.com/exchange/services/2006/messages" );
    ...
    public function SendMail( $liveId, $exchangeServiceAuthData, $to, $subject, $body )
    {
		...
		try
		{
            ...
            {
                // Get the SOAP Envelope that will be used to Send Email
                $soapEnvelope = $this->GetSendEmailEnvelope( $liveId, $serviceEndPoint, $authToken, $to, $subject,$body);
    ...
    public function CreateAppointment($liveId, $exchangeServiceAuthData, $to, $subject, $body, $start, $end, $isAllDayEvent)
    {
		...
		try
		{
            ...
            {
                // Get the SOAP Envelope that will be used to Send Email
                $soapEnvelope = $this->GetCreateAppointmentEnvelope( $liveId, $serviceEndPoint, $authToken, $to, $subject, $body, $start, $end, $isAllDayEvent);
                // POST The SOAP Envelope to the URL for the Exchange Server
                $curl = new CurlLib();
                $response = $curl->postSoapEnvelope( $serviceEndPoint,  $soapEnvelope, array('Content-Type: text/xml; charset=utf-8') );
    ...
    public function ResolveNames($liveId, $exchangeServiceAuthData, $unresolvedName)
    {
		...
		try
		{
            ...
            {
                // The the SOAP Envelope for the UnreadEmail count
                $soapEnvelope = $this->GetResolveNamesEnvelope( $liveId, $serviceEndPoint, $authToken, $unresolvedName);
                // POST The SOAP Envelope to the URL for the Exchange Server
                $curl = new CurlLib();
                $response = $curl->postSoapEnvelope( $serviceEndPoint, $soapEnvelope, array('Content-Type: text/xml; charset=utf-8') );
                $xml = simplexml_load_string($response);
...
/**************************************************************************************************************************/
     ...
     * @return <int>
     */
	private function GetUnreadEMailCount($impersonationSmtpAddress, $serviceEndPoint, $authToken)
	{
        $unreadCount = -1;
		try
		{
            ...
            {
                // The the SOAP Envelope for the UnreadEmail count
                $soapEnvelope = $this->GetUnreadEmailCountEnvelope( $impersonationSmtpAddress, $serviceEndPoint, $authToken );
     ...
     * @return <string>
     */
	private function GetUnreadEmailCountEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken)
	{
...
SOAP;
		...
		return $this->GetSoapEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, "FindItem", $soapBody);
     ...
     * @return <string>
     */
	private function GetUnreadEmailEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, $count)
	{
...
SOAP;
     ...
     * @return <string>
     */
	private function GetUpcomingCalendarItemsEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, $count)
	{
...
SOAP;
     ...
     * @return <string>
     */
    private function GetCreateAppointmentEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, $toEmailAddresses, $subject, $body, $start, $end, $isAllDayEvent)
    {
...
SOAP;
        ...
        return $this->GetSoapEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, "CreateItem", $soapBody);
    }

    private function GetSendEmailReplyEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, $id, $changeKey, $toEmailAddresses, $subject, $body)
    {
         $MailBox ="";
        foreach($toEmailAddresses as $address)
        {
...
SOAP;
    ...
    private function GetSendEmailForwardEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, $id, $changeKey, $toEmailAddresses, $subject, $body)
    {
        $MailBox ="";
        foreach($toEmailAddresses as $address)
        {
...
SOAP;
     ...
     * @return <string>
     */
    private function GetCreateEmailEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, $toEmailAddresses, $subject, $body)
    {
        $MailBox ="";
        foreach($toEmailAddresses as $address)
        {
...
SOAP;
     ...
     * @return <string>
     */
    private function GetSendEmailWithAttachmentsEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, $id, $changeKey)
    {
...
SOAP;
        return $this->GetSoapEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, "SendItem", $soapBody);
    }
     ...
     * @return <string>
     */
    private function GetSendEmailEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, $toEmailAddresses, $subject, $body)
    {
        $MailBox = "";
        foreach($toEmailAddresses as $address)
        {
...
/**
    ...
    * @return <type>
 */
    private function GetAttachmentEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, $attachmentId)
    {
        $soapBody = <<<SOAP
...
SOAP;
        return $this->GetSoapEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, "GetAttachment", $soapBody);
    }
     ...
     * @return <string>
     */
    private function GetViewEmailEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, $itemId,$changeKey)
    {
        $soapBody = <<<SOAP
    <m:GetItem>
      <m:ItemShape>
...
SOAP;
        return $this->GetSoapEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, "GetItem", $soapBody);
    }
     ...
     * @return <string>
     */
    private function GetViewCalendarEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, $itemId, $changeKey)
    {
        $soapBody = <<<SOAP
...
/**
     ...
     * @return <string>
 */
    private function GetCreateAttachmentEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken, $itemId, $changeKey,$name,$content)
    {
        $soapBody = <<<SOAP
...
SOAP;
     ...
     * @return <string>
     */
    private function GetResolveNamesEnvelope($impersonationSmtpAddress, $exchangeURL, $authToken,$unresolvedName)
    {
        $soapBody = <<<SOAP
...
SOAP;
     ...
     * @param <string> $impersonationSmtpAddress - the SMTP address for the impersonation account as set up in configuration
     * @param <string> $to - the URL of the exchange service
     * @param <string> $authToken - a SAML token
     * @param <string> $action - examples include "GetItem", "CreateItem", and "FindItem"
     * @param <string> $body - a SOAP body
     * @return <string>
     */
    private function GetSoapEnvelope($impersonationSmtpAddress, $to, $authToken, $action, $body)
	{
        $actionNode = "";
        if( strlen($action) > 0 )
        {
            $actionNode = "<wsa:Action soap:mustUnderstand='1'>http://schemas.microsoft.com/exchange/services/2006/messages/$action</wsa:Action>";
        }

        $toNode = "";
        if( strlen($to) > 0 )
        {
            $toNode = "<wsa:To soap:mustUnderstand='1'>$to</wsa:To>";
        }
    ...
    <wsse:Security soap:mustUnderstand='1'>$authToken</wsse:Security>
