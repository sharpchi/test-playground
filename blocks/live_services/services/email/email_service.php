...
class EmailService {
    ...
    function __construct() {
        ...
        if( @$CFG->block_live_services_useEWS == 1 ) {
            ...
            $this->ews_auth = new EWSAuthentication($impersonationLiveId, $impersonationPassword);
        }
    }
    /**
	...
	if( $CFG->block_live_services_showCalendar == 0 && $CFG->block_live_services_showEmail == 0 ) return "";
        ...
        if( @$CFG->block_live_services_useEWS == 1 && isset($this->ews_auth) ) 
        {
            // Get the Exchange Server Authentication Data
            $exchangeServiceData = $this->ews_auth->AuthenticateAgainstEWSEndPoint( false );
            
            // Get mail from Exchange Server. If there is no mail, display Hotmail version
            $emailData = $this->GetEmail( $loggedInLiveId, $exchangeServiceData );
		    ...
		    <div class="msm_collapsible" id="msmEmailAndCalendar" style="display:block">
		    	<div id="owaLinksContainer">
                    ...
                    if(!$refresh) 
                    {
                        ...
                        $returnData .= $this->GetLoadingContactsMessage();
                    ...
                    if( $CFG->block_live_services_showEmail ) 
                    {
                        $returnData .=   $this->RenderEmail( $loggedInLiveId, $emailData );
                    }
                    if( $CFG->block_live_services_showCalendar ) 
                    {
                        $calendarData = $this->GetCalendar( $loggedInLiveId, $exchangeServiceData );
                        $returnData .=  $this->RenderCalendar( $loggedInLiveId, $calendarData );
                    }
                    if(!$refresh) 
                    {
    ...
    private function GetQuickMessageLink() {
        //only show link if contacts have been loaded into session
        //otherwise, there will be a delay when resolving names
        if(@$_SESSION['msm_contacts']===null) {
            $quickMessageDisplay = "none";
        }
        else {
           $quickMessageDisplay = "inline";
        }
        $emailQuickMessage = getLocalizedString('emailQuickMessage');
        return '<span id="quickMessageSection" style="display:'.$quickMessageDisplay.'">
                    |
    ...
    private function GetQuickEventLink() {
        ...
        //otherwise, there will be a delay when resolving names
        if(@$_SESSION['msm_contacts']===null) {
            $quickEventDisplay = "none";
        }
        else {
           $quickEventDisplay = "inline";
        }
        $calendarQuickItem = getLocalizedString('calendarQuickItem');
        return '<span id="quickEventSection" style="display:'.$quickEventDisplay.';">
        |
        <a href="#" id="showAppointment">'.$calendarQuickItem.'</a></span>';
    }
    /**
     * Returns a message saying that contacts are being loaded.
     * This displayed while the AJAX call to load contacts into session is processing
     */
    private function GetLoadingContactsMessage() {
        //only show message if contacts have not been loaded into session
        if(@$_SESSION['msm_contacts']===null) {
            $loadingContactsDisplay = "inline";
        }
        else {
            $loadingContactsDisplay = "none";
        }
        $loadingContactsText = getLocalizedString('emailLoadingContacts');
        return '<span id="loadingContacts" style="display"'.$loadingContactsDisplay.';"><em>&nbsp;&nbsp;'.$loadingContactsText.'</em></span>';
    }
    ...
    private function GetCalendar($liveId, $exchangeServiceData) {
        ...
        try {
            ...
            handleException($e);		
            $eventCount = -1;
            if( strpos(@$e->getFaultCode(),"InvalidSecurityToken" )) {
                $exchangeServiceData    = $this->ews_auth->AuthenticateAgainstEWSEndPoint( true );
                $calendarItemsResponse  = $ewsWrapper->GetUpcomingCalendarItems( $liveId, $exchangeServiceData, 3 );
                return $calendarItemsResponse;
            }
        }
